...
JSON:{"title":"tên CHÍNH XÁC bài được yêu cầu","artist":"","lyrics":"TOÀN BỘ lyrics có [Verse 1][Chorus][Bridge]","genre":"","mood":"","tempo":"slow/medium/fast","bpm":120,"key":"","instruments":"","vocal":"","production":"","confidence":"high/low"}`,
  ...
  const handleRewrite=useCallback(async()=>{
    ...
    // Build strict line-by-line guide
    let guide="";let lineCount=0;
    origLines.forEach(l=>{
      if(l.match(/^\[/)){guide+=`${l}\n`}
      else if(l.match(/^\(/)||!l.trim()){/* skip */}
      else{lineCount++;const syl=countSyl(l);guide+=`→ viết ${syl} âm tiết (gốc: ${l.substring(0,35)}${l.length>35?"...":""})\n`}
    });

    const lRaw=await callAI(`Nhạc sĩ Việt. Viết ĐÚNG ${lineCount} câu, mỗi câu ĐÚNG số âm tiết. CHỈ JSON.`,
`VIẾT LỜI VIỆT: "${song.title}"${song.artist?" - "+song.artist:""}
${genre} | ${mood} | ${vocal}
${notes?`Yêu cầu: ${notes}`:""}

CẤU TRÚC BẮT BUỘC (viết ĐÚNG số câu, ĐÚNG số âm tiết):
${guide}
LUẬT:
1. GIỮ NGUYÊN section markers [Verse 1] [Chorus] [Bridge] etc.
2. Mỗi section có ĐÚNG SỐ CÂU như gốc — KHÔNG thêm, KHÔNG bớt
3. Mỗi câu ĐÚNG số âm tiết chỉ định (±1 tối đa)
4. Vần cuối mỗi cặp câu (AABB): đi-gì, yêu-nhiều, đau-nhau
5. Xưng hô: tình yêu→anh/em, bạn bè→bạn/tôi, tự sự→ta
6. Ngôn ngữ thơ, gợi cảm — KHÔNG giọng dịch
7. 100% tiếng Việt
8. Mỗi câu trên 1 dòng riêng

VÍ DỤ (8 âm tiết):
gốc: "I just woke up from a dream" (8)
→ "Anh vừa tỉnh giấc trong đêm dài" (8) ✓ — đủ 8, có vần, có ý nghĩa

JSON: {"title":"tên Việt","lyrics":"[Verse 1]\\ncâu 1\\ncâu 2\\n[Chorus]\\ncâu 3\\ncâu 4"}`,
  ...
  const handleRegen=async sec=>{
    if(!viet||!ak)return;setRegenSec(sec);try{
      ...
      let secGuide="";
      origLines.forEach(l=>{
        if(l.toLowerCase().includes(`[${sec.toLowerCase()}`)){inS=true;origSec+=l+"\n"}
        else if(inS&&l.match(/^\[/))inS=false;
        else if(inS){origSec+=l+"\n";if(l.trim()&&!l.match(/^\(/)){const syl=countSyl(l);secGuide+=`→ viết ${syl} âm tiết\n`}}
      });
      const raw=await callAI("Nhạc sĩ Việt. CHỈ trả lyrics, mỗi câu 1 dòng.",
`Viết lại [${sec}] bài "${viet.title}" (${genre}, ${mood}).
Số câu gốc + âm tiết:
${secGuide||"(tham khảo bài hiện tại)"}
Bài hiện tại:\n${viet.lyrics}

Viết ĐÚNG số câu, ĐÚNG số âm tiết ±1. Vần cuối mỗi cặp câu. Giữ xưng hô nhất quán với bài. 100% tiếng Việt.
CHỈ trả lyrics cho [${sec}], KHÔNG header, KHÔNG giải thích.`,
        ...
        <div style={{marginTop:16,paddingTop:14,borderTop:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <button className="btn bs" onClick={()=>{
              ...
              // Build line-by-line guide for external AI
              let extGuide="";
              song.lyrics.split("\n").forEach(l=>{
                if(l.match(/^\[/))extGuide+=l+"\n";
                else if(l.match(/^\(/)||!l.trim()){}
                else{const syl=countSyl(l);extGuide+=`${l} → viết ${syl} âm tiết\n`}
              });
              const p=`Viết lời Việt cho bài hát. BẠN LÀ NHẠC SĨ, KHÔNG PHẢI DỊCH GIẢ.

BÀI GỐC: "${song.title}"${song.artist?" - "+song.artist:""}
${genre} | ${mood} | ${vocal}

CẤU TRÚC (viết ĐÚNG số câu, ĐÚNG số âm tiết mỗi câu):
${extGuide}
QUY TẮC:
1. GIỮ NGUYÊN [Verse 1] [Chorus] [Bridge] — ĐÚNG số câu mỗi section
2. Mỗi câu ĐÚNG số âm tiết chỉ định (±1)
3. Vần cuối mỗi cặp câu: đi-gì, yêu-nhiều, đau-nhau
4. Xưng hô: tình yêu→anh/em, bạn bè→bạn/tôi, tự sự→ta
5. Ngôn ngữ thơ, gợi cảm — KHÔNG dịch literal, viết lại hoàn toàn
6. 100% tiếng Việt

VÍ DỤ:
"I just woke up from a dream" (8) → "Anh vừa tỉnh giấc trong đêm dài" (8) ✓

Trả về: Tên bài Việt + Full lyrics (giữ section markers)`;
